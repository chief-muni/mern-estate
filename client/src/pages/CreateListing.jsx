...
function CreateListing() {
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      ...
      setIsLoading(true);
      setError(false);
      const { data } = await axios.post('/listing/create', formData);
      setIsLoading(false);
      if(!data) {
    ...
    <main className="p-3 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-6">
        ...
        <div className="flex flex-col gap-4 flex-1">
          ...
          <button type="submit" className="mt-4" disabled={isLoading || isUploading}>{isLoading ? 'Creating...' : `Create Listing`}</button>
